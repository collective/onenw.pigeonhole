...
class IRelatedNewsPortlet(IPortletDataProvider):

    portlet_title = schema.TextLine(title=u"Title for the portlet",
                                    default=u"Related News",
                                    required=True)
...
class Assignment(base.Assignment):
    implements(IRelatedNewsPortlet)
    ...
    @property
    def title(self):
        return u"Related News Portlet"

class Renderer(base.Renderer):

    render = ViewPageTemplateFile('relatednews.pt')
    ...
    def css_class(self):
        ...
        return "portlet-related-news-%s" % normalizer.normalize(header)
    ...
    def _data(self):
        ...
        query = {'sort_on': 'Date',
                 'sort_order': 'reverse',
                 'portal_type': ('News Item','PressClip', 'PressRelease'),
...
class AddForm(base.AddForm):
    form_fields = form.Fields(IRelatedNewsPortlet)
    label = u"Add Related News portlet"
    description = u'This portlet displays recent news that are considered "related" to the current page.'
...
class EditForm(base.EditForm):
    form_fields = form.Fields(IRelatedNewsPortlet)
    label = u"Edit Related News portlet"
    description = u'This portlet displays recent news that are considered "related" to the current page.'
    form_fields['which_fields_relate'].custom_widget = PHMultiCheckBoxWidget
